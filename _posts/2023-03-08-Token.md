...
lastmod: 2023-03-08 23:20:00 +900
sitemap:
  changefreq: monthly
  priority: 0.5
...
( 다른 방식으로는 [Cookie & Session](/posts/cookie-&-session/){:target="_blank"}을 이용한 방식이 있습니다. )<br />

## 0️⃣ Token vs Session
`Token`에 대해 알아보기 위해서는 [Cookie & Session](/posts/cookie-&-session/){:target="_blank"}과 비교해서 이해하면 도움이 됩니다.<br />
...
## 0️⃣ AccessToken
인증처리를 하는데 사용하는 `Token`입니다.<br />

## 1️⃣ RefreshToken
`AccessToken`의 유효기간이 지난 경우 재발급하는데 사용할 `Token`입니다.<br />

## 2️⃣ 동작흐름
상황에 따라서 여러 방법이 있겠지만 `Cookie`를 통해서 `Token`을 전달하고 `RefreshToken` 또한 브라우저에 전달한다고 가정하고 설명하겠습니다.<br />

1. 유저로 인증이 되었다면 해당 유저의 식별자를 `Payload`로 갖는 `AccessToken`, `RefreshToken`을 생성
2. 생성한 `Token`을 `Cookie`에 동봉해서 브라우저로 전달
...
# 📮 레퍼런스
1. [Cookie & Session](/posts/cookie-&-session/){:target="_blank"}