...
위 모델을 그대로 선언하고 [명령어](/posts/prisma/#3-명령어)들을 순서대로 실행하면서 변화 과정을 테스트해보시면 이해에 도움이 됩니다.<br />
...
## 2. seed 코드 작성 예시
아래 코드는 [위의 예시](/posts/prisma/#1-모델-정의)의 `Profile`을 예로 `seed`를 만들어봤습니다.<br />
...
[위의 예시](/posts/prisma/#1-모델-정의)의 `Profile`을 예로 들어보겠습니다.
...
# 📮 레퍼런스
1. [Prisma migrate - 블로그](https://pyh.netlify.app/prisma/prisma_migrate){:target="_blank"}
