...
> 해당 게시글은 자바스크립트 실행 컨텍스트에 대해 공부한 내용을 정리한 포스트입니다.<br />실행 컨텍스트와 연관된 `JS`의 주요 개념들에 대해서도 정리했습니다.
{: .prompt-info }
...
[환경 레코드(`Environment Record`)](/posts/실행-컨텍스트/#4-환경-레코드--environment-record-)와 [외부 렉시컬 환경(`Outer Lexical Environment`)](/posts/실행-컨텍스트/#5-외부-렉시컬-환경-outer-lexical-environment-)을 갖습니다.<br />

함수를 실행하는 경우 `Lexical Environment`가 생성되며 `Environment Record`와 `Outer Lexical Environment` 값이 등록됩니다.<br />
...
외부 렉시컬 환경(`Outer Lexical Environment`)은 `null`을 갖습니다.<br />
...
### 5. 외부 렉시컬 환경( Outer Lexical Environment )
`Lexical Environment`의 프로퍼티이며, 외부 렉시컬 환경에 대한 참조를 갖습니다.<br />
쉽게 말해 상위 스코프의 참조를 갖는다는 의미입니다.<br />

추가로 아래 예시를 설명하자면 `Global Lexical Environment`의 `Environment Record`에 `outer()`가 속하고 `Outer Lexical Environment`는 `null`을 갖습니다.<br />
`outer()`가 실행되면 `Lexical Environment`의 `Environment Record`에 `inner()`가 등록되고 `Outer Lexical Environment`가 `Global Lexical Environment`를 참조합니다.<br />
`inner()`가 실행되면 `Lexical Environment`의 `Environment Record`에 `inside()`가 등록되고 `Outer Lexical Environment`가 `outer()`를 참조합니다.<br />
`inside()`가 실행되면 `Lexical Environment`의 `Outer Lexical Environment`가 `inner()`를 참조합니다.<br />

+ 요약
1. `outer()`는 `Lexical Environment`의 `Outer Lexical Environment`를 통해서 `Global Lexical Environment`를 참조한다.
1. `inner()`는 `Lexical Environment`의 `Outer Lexical Environment`를 통해서 `outer()`를 참조한다.
1. `inside()`는 `Lexical Environment`의 `Outer Lexical Environment`를 통해서 `inner()`를 참조한다.

따라서 내부 스코프는 외부 스코프를 참조할 수 있게 됩니다.<br />
이런 원리로 동작하는 것이 [스코프 체인](/posts/실행-컨텍스트/#스코프와-스코프-체인)입니다.<br />

```js
// "Global Lexical Environment"의 "Outer Lexical Environment"는 "null"을 갖습니다.

const outer = () => {
  // 해당 "Lexical Environment"의 "Outer Lexical Environment"는 "Global Lexical Environment"를 참조합니다.

  const inner = () => {
    // 해당 "Lexical Environment"의 "Outer Lexical Environment"는 "outer()"를 참조합니다.

    const inside = () => {
      // 해당 "Lexical Environment"의 "Outer Lexical Environment"는 "inner()"를 참조합니다.

      // 따라서 여기서 특정 식별자에 접근하면
      // 1. 현재 "Lexical Environment"인 "inside()" 검색
      // 2. 현재 "Lexical Environment"의 "Outer Lexical Environment"인 "inner()" 검색
      // 3. "inner()"의 "Outer Lexical Environment"인 "outer()" 검색
      // 4. "outer()"의 "Outer Lexical Environment"인 "Global Lexical Environment" 검색
...
즉, `Lexical Environment`를 하나의 스코프라고 볼 수 있습니다.<br />

다음은 스코프 체인에 대해 살펴보겠습니다.<br />
`Lexical Environment`는 내부 값인 `Outer Lexical Environment`를 통해서 외부 렉시컬 환경에 대한 참조 기억합니다.<br />
코드가 실행되면서 식별자를 만나는 경우 먼저 `Lexical Environment`의 `Environment Record`에서 찾습니다.<br />
그리고 찾지 못한다면 `Outer Lexical Environment`를 통해 외부 `Lexical Environment`의 `Environment Record`에 접근하여 식별자를 찾는 과정을 반복합니다.<br />
이 과정을 반복하면서 `Global Lexical Environment`의 `Environment Record`까지 식별자를 찾는 것을 스코프 체인이라고 합니다.<br />

+ 요약
1. 식별자 검색 시 현재 `Lexical Environment`의 `Environment Record`에서 찾습니다.
2. 못 찾았다면 현재 `Lexical Environment`의 `Outer Lexical Environment`을 통해서 외부 `Lexical Environment`의 `Environment Record`에서 찾습니다.
3. 찾을 때까지 2번의 과정을 반복하다가 `Global Lexical Environment`의 `Environment Record`에서도 못 찾는다면 오류를 냅니다.
...
여기서 **어휘적 환경**이란 [`Lexical Environment`](/posts/실행-컨텍스트/#2-렉시컬-환경--lexical-environment-) 즉, 해당 스코프의 환경에 대한 정보를 저장한 객체입니다.<br />
...
따라서 `outer`의 어휘적 환경인 `Lexical Environment`(정확히는 `Lexical Environment`의 `Environment Record`)에서 선언된 `v`는 해제되지 않고 접근이 가능합니다.<br />
이때 `v`를 **자유 변수**라고 하고, `inner()`를 **클로저**라고 합니다.<br />

추가로 `예시2`를 보시면 `v`가 증가하지 않고 같은 값만 출력하는 것처럼 보입니다.<br />
곰곰히 생각해보면 `outer()`가 실행되는 시점에 하나의 `Lexical Environment`가 생성되고 그 내부에 `Environment Record`에 `v`가 등록됩니다.<br />
따라서 `outer()`의 실행마다 독립적인 `Lexical Environment`가 생성되기 때문에 서로 다른 `v`를 갖기 때문에 `v`가 증가하지 않는 것처럼 보이는 것입니다.<br />

근데 여기서 또 하나의 의문이 생길 수 있습니다.<br />
`inner()`가 실행되는 시점 즉, 코드상에서는 `func()`인 시점에 `inner()`의 `Lexical Environment`가 생성됩니다.<br />
`inner()`의 `Lexical Environment`에서 접근할 수 있는 다른 `Lexical Environment`는 `Global Lexical Environment`밖에 없습니다.<br />
근데 어떻게 `func()`를 통해서 `v`에 접근할 수 있을까요?<br />
바로 `JavaScript`의 함수가 갖는 [`[[Environment]]`](/posts/실행-컨텍스트/#6-environment)을 이용하기 때문입니다.<br />
즉, 생성한 시점의 `Lexical Environment`의 참조를 알기 때문에 다른 곳에서 실행해도 찾아갈 수 있습니다.<br />
...
그래서 실행 컨텍스트를 공부하고 이해하려면 `Lexical Environment`, `Outer Lexical Environment` 등의 개념으로 이어졌습니다.<br />
